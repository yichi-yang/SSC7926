...
volatile bool BufferDMA::second_half = true;
...
void dma_half_complete_isr()
{
    ...
    BufferDMA::second_half = !BufferDMA::second_half;
}
...
volatile int16_t *BufferDMA::read_half()
{
    data_ready--;
    return p_elems + b_size / 2 * second_half;
}

volatile size_t BufferDMA::remain()
{
    return (b_size - dmaChannel->TCD->CITER) % (b_size / 2);
}